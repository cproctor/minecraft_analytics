...
export default class VoxelChunk {
  ...
  constructor(cx, cy, cz, opLimit, world) {
    ...
    this.center = this.bb0.clone().add(this.size.clone().divideScalar(2));
    this.voxels = new Map();
  }
  ...
  createObjects() {
    const terrain = new MeshLambertMaterial({color:"#27ae60"});
  ...
  setGeometry(geometry, positions, normals, indices) {
    ...
    const c = this.center.toArray();
    for (var i = 0; i < geometryPositions.length; i++) {
      geometryPositions[i] = i < positions.length ? positions[i] : c[i % 3];
      geometryNormals[i] = i < normals.length ? normals[i] : 0;
    }
  ...
  bufferSpaceForOps() {
    ...
    const extra = 3 * 8; // I don't know why, but one test was overflowing
    return this.opLimit * facesPerVoxel * trianglesPerFace * verticesPerTriangle + extra;
  }
