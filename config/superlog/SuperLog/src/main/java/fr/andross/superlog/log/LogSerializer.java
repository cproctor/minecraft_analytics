...
public final class LogSerializer {
    ...
    protected void serialize(@NotNull final Entity e, @Nullable final String fieldName) {
		...
		case "EYEDIR":
                    ...
                    p = ((Player) e);
		    loc = p.getLocation();
		    value = serializeDirection(loc);
		    break;
		case "TARGETBLOCK":
                    if (!(e instanceof Player)) break;
                    p = ((Player) e);
		    loc = p.getLocation();
		    final Block b = p.getTargetBlock((Set) null, MAX_TARGET_DISTANCE);
		    final Location bloc = b.getLocation();
		    if (bloc.distance(loc) < MAX_TARGET_DISTANCE) {
		      value = serializeLocation(bloc);
		    }
		    break;
    ...
    public String serializeLocation(@NotNull final Location loc) {
	return "[" + loc.getBlockX() + ", " + loc.getBlockY() + ", " + loc.getBlockZ() + "]";
    }
    public String serializeDirection(@NotNull final Location loc) {
	return "[" + loc.getPitch() + ", " + loc.getYaw() + "]";
    }
}
